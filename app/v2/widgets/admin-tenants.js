/**
 * @file widgets/admin-tenants.js
 * @description Admin · multi-tenant orgs (SaaS only)
 * @author  Sachin Kumar
 * @license MIT
 */
...
export async function init(opts) {
  ...
  if (!root) throw new Error('admin-tenants: mount not found');

  const dom = document.createElement('div');
  dom.className = 'pt-admin-tenants';
  ...
  dom.innerHTML = `
    <div style="font:600 11px Inter;color:#E11D48;letter-spacing:3px;margin-bottom:6px">ADMIN-TENANTS</div>
    <div style="font:600 18px Georgia;margin-bottom:8px">Admin · multi-tenant orgs (SaaS only)</div>
