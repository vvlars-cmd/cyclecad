/**
 * @file widgets/sketch-constraint-solver.js
 * @description Sketch · 12-constraint relaxation solver
 * @author  Sachin Kumar
 * @license MIT
 */
...
export async function init(opts) {
  ...
  if (!root) throw new Error('sketch-constraint-solver: mount not found');

  const dom = document.createElement('div');
  dom.className = 'pt-sketch-constraint-solver';
  ...
  dom.innerHTML = `
    <div style="font:600 11px Inter;color:#D4A843;letter-spacing:3px;margin-bottom:6px">SKETCH-CONSTRAINT-SOLVER</div>
    <div style="font:600 18px Georgia;margin-bottom:8px">Sketch · 12-constraint relaxation solver</div>
    ...
    <button data-run style="background:#D4A843;color:#fff;border:none;padding:6px 14px;border-radius:4px;font:600 11px Inter;cursor:pointer">RUN</button>
  ...
  async function run(params = {}) {
    ...
    const result = {
      ...
      params, widget: 'sketch-constraint-solver',
    };
