/**
 * @file widgets/sketch-rect.js
 * @description Sketch · 2D rect on the XY plane. Emits `entity` event.
 * @author  Sachin Kumar
 * @license MIT
 */
...
export async function init(opts) {
  ...
  if (!scene) throw new Error('sketch-rect: opts.scene required');
  ...
  function makeShape(params) {
    let geom;
    const w = params.w || 10; const h = params.h || 6; const pts = [[-w/2,-h/2],[w/2,-h/2],[w/2,h/2],[-w/2,h/2],[-w/2,-h/2]].map(p=>new THREE.Vector3(p[0],p[1],0)); geom = new THREE.BufferGeometry().setFromPoints(pts);
    const line = new THREE.LineLoop(geom, new THREE.LineBasicMaterial({ color: 0xffd000 }));
    line.name = 'pt-sketch-rect';
    scene.add(line);
    entities.push(line);
    emit('entity', { shape: 'rect', line, params });
    return line;
  }
