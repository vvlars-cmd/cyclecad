/**
 * @file widgets/standards-id.js
 * @description Doc · DIN / ISO standard identifier
 * @author  Sachin Kumar
 * @license MIT
 */
...
export async function init(opts) {
  ...
  if (!root) throw new Error('standards-id: mount not found');

  const dom = document.createElement('div');
  dom.className = 'pt-standards-id';
  ...
  dom.innerHTML = `
    <div style="font:600 11px Inter;color:#3AAFA9;letter-spacing:3px;margin-bottom:6px">STANDARDS-ID</div>
    <div style="font:600 18px Georgia;margin-bottom:8px">Doc · DIN / ISO standard identifier</div>
